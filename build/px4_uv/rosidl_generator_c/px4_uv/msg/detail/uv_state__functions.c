// generated from rosidl_generator_c/resource/idl__functions.c.em
// with input from px4_uv:msg/UvState.idl
// generated code does not contain a copyright notice
#include "px4_uv/msg/detail/uv_state__functions.h"

#include <assert.h>
#include <stdbool.h>
#include <stdlib.h>
#include <string.h>

#include "rcutils/allocator.h"


// Include directives for member types
// Member `pos`
// Member `vel`
#include "geometry_msgs/msg/detail/vector3__functions.h"

bool
px4_uv__msg__UvState__init(px4_uv__msg__UvState * msg)
{
  if (!msg) {
    return false;
  }
  // pos
  if (!geometry_msgs__msg__Vector3__init(&msg->pos)) {
    px4_uv__msg__UvState__fini(msg);
    return false;
  }
  // vel
  if (!geometry_msgs__msg__Vector3__init(&msg->vel)) {
    px4_uv__msg__UvState__fini(msg);
    return false;
  }
  // current_yaw
  // ref_lat
  // ref_lon
  // ref_alt
  // battery
  // status
  return true;
}

void
px4_uv__msg__UvState__fini(px4_uv__msg__UvState * msg)
{
  if (!msg) {
    return;
  }
  // pos
  geometry_msgs__msg__Vector3__fini(&msg->pos);
  // vel
  geometry_msgs__msg__Vector3__fini(&msg->vel);
  // current_yaw
  // ref_lat
  // ref_lon
  // ref_alt
  // battery
  // status
}

bool
px4_uv__msg__UvState__are_equal(const px4_uv__msg__UvState * lhs, const px4_uv__msg__UvState * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  // pos
  if (!geometry_msgs__msg__Vector3__are_equal(
      &(lhs->pos), &(rhs->pos)))
  {
    return false;
  }
  // vel
  if (!geometry_msgs__msg__Vector3__are_equal(
      &(lhs->vel), &(rhs->vel)))
  {
    return false;
  }
  // current_yaw
  if (lhs->current_yaw != rhs->current_yaw) {
    return false;
  }
  // ref_lat
  if (lhs->ref_lat != rhs->ref_lat) {
    return false;
  }
  // ref_lon
  if (lhs->ref_lon != rhs->ref_lon) {
    return false;
  }
  // ref_alt
  if (lhs->ref_alt != rhs->ref_alt) {
    return false;
  }
  // battery
  if (lhs->battery != rhs->battery) {
    return false;
  }
  // status
  if (lhs->status != rhs->status) {
    return false;
  }
  return true;
}

bool
px4_uv__msg__UvState__copy(
  const px4_uv__msg__UvState * input,
  px4_uv__msg__UvState * output)
{
  if (!input || !output) {
    return false;
  }
  // pos
  if (!geometry_msgs__msg__Vector3__copy(
      &(input->pos), &(output->pos)))
  {
    return false;
  }
  // vel
  if (!geometry_msgs__msg__Vector3__copy(
      &(input->vel), &(output->vel)))
  {
    return false;
  }
  // current_yaw
  output->current_yaw = input->current_yaw;
  // ref_lat
  output->ref_lat = input->ref_lat;
  // ref_lon
  output->ref_lon = input->ref_lon;
  // ref_alt
  output->ref_alt = input->ref_alt;
  // battery
  output->battery = input->battery;
  // status
  output->status = input->status;
  return true;
}

px4_uv__msg__UvState *
px4_uv__msg__UvState__create()
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  px4_uv__msg__UvState * msg = (px4_uv__msg__UvState *)allocator.allocate(sizeof(px4_uv__msg__UvState), allocator.state);
  if (!msg) {
    return NULL;
  }
  memset(msg, 0, sizeof(px4_uv__msg__UvState));
  bool success = px4_uv__msg__UvState__init(msg);
  if (!success) {
    allocator.deallocate(msg, allocator.state);
    return NULL;
  }
  return msg;
}

void
px4_uv__msg__UvState__destroy(px4_uv__msg__UvState * msg)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (msg) {
    px4_uv__msg__UvState__fini(msg);
  }
  allocator.deallocate(msg, allocator.state);
}


bool
px4_uv__msg__UvState__Sequence__init(px4_uv__msg__UvState__Sequence * array, size_t size)
{
  if (!array) {
    return false;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  px4_uv__msg__UvState * data = NULL;

  if (size) {
    data = (px4_uv__msg__UvState *)allocator.zero_allocate(size, sizeof(px4_uv__msg__UvState), allocator.state);
    if (!data) {
      return false;
    }
    // initialize all array elements
    size_t i;
    for (i = 0; i < size; ++i) {
      bool success = px4_uv__msg__UvState__init(&data[i]);
      if (!success) {
        break;
      }
    }
    if (i < size) {
      // if initialization failed finalize the already initialized array elements
      for (; i > 0; --i) {
        px4_uv__msg__UvState__fini(&data[i - 1]);
      }
      allocator.deallocate(data, allocator.state);
      return false;
    }
  }
  array->data = data;
  array->size = size;
  array->capacity = size;
  return true;
}

void
px4_uv__msg__UvState__Sequence__fini(px4_uv__msg__UvState__Sequence * array)
{
  if (!array) {
    return;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();

  if (array->data) {
    // ensure that data and capacity values are consistent
    assert(array->capacity > 0);
    // finalize all array elements
    for (size_t i = 0; i < array->capacity; ++i) {
      px4_uv__msg__UvState__fini(&array->data[i]);
    }
    allocator.deallocate(array->data, allocator.state);
    array->data = NULL;
    array->size = 0;
    array->capacity = 0;
  } else {
    // ensure that data, size, and capacity values are consistent
    assert(0 == array->size);
    assert(0 == array->capacity);
  }
}

px4_uv__msg__UvState__Sequence *
px4_uv__msg__UvState__Sequence__create(size_t size)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  px4_uv__msg__UvState__Sequence * array = (px4_uv__msg__UvState__Sequence *)allocator.allocate(sizeof(px4_uv__msg__UvState__Sequence), allocator.state);
  if (!array) {
    return NULL;
  }
  bool success = px4_uv__msg__UvState__Sequence__init(array, size);
  if (!success) {
    allocator.deallocate(array, allocator.state);
    return NULL;
  }
  return array;
}

void
px4_uv__msg__UvState__Sequence__destroy(px4_uv__msg__UvState__Sequence * array)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (array) {
    px4_uv__msg__UvState__Sequence__fini(array);
  }
  allocator.deallocate(array, allocator.state);
}

bool
px4_uv__msg__UvState__Sequence__are_equal(const px4_uv__msg__UvState__Sequence * lhs, const px4_uv__msg__UvState__Sequence * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  if (lhs->size != rhs->size) {
    return false;
  }
  for (size_t i = 0; i < lhs->size; ++i) {
    if (!px4_uv__msg__UvState__are_equal(&(lhs->data[i]), &(rhs->data[i]))) {
      return false;
    }
  }
  return true;
}

bool
px4_uv__msg__UvState__Sequence__copy(
  const px4_uv__msg__UvState__Sequence * input,
  px4_uv__msg__UvState__Sequence * output)
{
  if (!input || !output) {
    return false;
  }
  if (output->capacity < input->size) {
    const size_t allocation_size =
      input->size * sizeof(px4_uv__msg__UvState);
    rcutils_allocator_t allocator = rcutils_get_default_allocator();
    px4_uv__msg__UvState * data =
      (px4_uv__msg__UvState *)allocator.reallocate(
      output->data, allocation_size, allocator.state);
    if (!data) {
      return false;
    }
    // If reallocation succeeded, memory may or may not have been moved
    // to fulfill the allocation request, invalidating output->data.
    output->data = data;
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!px4_uv__msg__UvState__init(&output->data[i])) {
        // If initialization of any new item fails, roll back
        // all previously initialized items. Existing items
        // in output are to be left unmodified.
        for (; i-- > output->capacity; ) {
          px4_uv__msg__UvState__fini(&output->data[i]);
        }
        return false;
      }
    }
    output->capacity = input->size;
  }
  output->size = input->size;
  for (size_t i = 0; i < input->size; ++i) {
    if (!px4_uv__msg__UvState__copy(
        &(input->data[i]), &(output->data[i])))
    {
      return false;
    }
  }
  return true;
}
